...
class Data:
    ...
    def __init__(self, r):
        ...
        self.province_to_cities = (self.transfer_pro_to_ct(self.data))[
            'city']  # Series province->city(string，空格隔开)
    ...
    def get_latest_data(self, filename='latest.json'):
        ...
        '''
        f = open('assets/data/%s' % filename, 'r', encoding='utf-8')
        for line in f:
            dic = json.loads(line)
    ...
    def push_cty_pro_to_update(self, update_cities):
        ...
        for update_city in update_cities:
            ...
            if update_city in all_keys:
                city_to_uid[update_city].extend(
                    self.r.get_subscribed_users(update_city))
            if ALL_CHINA in all_keys:
                city_to_uid[update_city].extend(
                    self.r.get_subscribed_users(ALL_CHINA))
